export interface CarteVerteFormData {
...
export const submitCarteVerteForm = async (
  formData: CarteVerteFormData
): Promise<{
  success: boolean;
  message: string;
  error?: string;
  ...
  try {
    ...
    if (result.success) {
      return {
        success: true,
        message: "Votre demande de carte verte a été envoyée avec succès !",
      };
    } else {
      return {
        success: false,
    ...
    console.error("Carte verte form submission error:", error);
    return {
      success: false,
      message:
